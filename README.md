...
If crash recovery blocks startup, the launch task stays `running` with phase `waiting_user_action`, matching UnrealEditor PID/window evidence, and a poll command. Choose **Restore Selected** or **Skip Restore** in that existing editor; the same launch task resumes and can complete. Its configured launch timeout still applies. The CLI does not click this dialog. If the Agent started and closed the previous session, it may handle the window through UI automation on the next launch after verifying the project, PID/window, and listed packages. Recover needed unsaved work, or skip recovery when the prior task decision already permits discarding it; do not ask again for an already authorized choice. Ask when ownership or the intended choice is unclear. See the [startup-recovery rule](cli_anything/unreal/skills/references/safety.md#restore-packages-after-an-agent-managed-restart).
